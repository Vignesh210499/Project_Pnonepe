...
Map_trans = pd.DataFrame(table3,columns=("States","Years","Quarter","Transaction_name","Transaction_count","Transaction_amount"))
...
def reg_state_all_TA(sel_year,state):
    year= int(sel_year)
    mts= Map_trans[["States", "Years","Transaction_name", "Transaction_amount"]]
    mts1= mts.loc[(Map_trans["States"]==state)&(Map_trans["Years"]==year)]
    mts2= mts1.groupby("Transaction_name")["Transaction_amount"].sum()
    mts3= pd.DataFrame(mts2).reset_index()

    fig_mts= px.bar(mts3, x= "Transaction_name", y= "Transaction_amount", title= "DISTRICT and TRANSACTION AMOUNT",
                    color_discrete_sequence= px.colors.sequential.Darkmint_r)
    fig_mts.update_layout(width= 600, height= 500)
    return st.plotly_chart(fig_mts)

def ques1():
    brand= Aggre_user[["Brand","Transaction_count"]]
...
def ques3():
    htd = Map_trans[["Transaction_name","Transaction_amount"]]
    htd1 = htd.groupby("Transaction_name")["Transaction_amount"].sum().sort_values(ascending=False)
    htd2 = pd.DataFrame(htd1).head(10).reset_index()

    fig_htd = px.pie(htd2,values="Transaction_amount",names="Transaction_name",title="TOP 10 DISTRICTS OF HIGHEST TRANSACTION AMOUNT",
                    color_discrete_sequence=px.colors.sequential.Emrld_r)
    return st.plotly_chart(fig_htd)

def ques4():
    htd= Map_trans[["Transaction_name", "Transaction_amount"]]
    htd1= htd.groupby("Transaction_name")["Transaction_amount"].sum().sort_values(ascending=True)
    htd2= pd.DataFrame(htd1).head(10).reset_index()

    fig_htd= px.pie(htd2, values= "Transaction_amount", names= "Transaction_name", title="TOP 10 DISTRICTS OF LOWEST TRANSACTION AMOUNT",
...
def ques10():
    dt= Map_trans[["Transaction_name", "Transaction_amount"]]
    dt1= dt.groupby("Transaction_name")["Transaction_amount"].sum().sort_values(ascending=True)
    dt2= pd.DataFrame(dt1).reset_index().head(50)

    fig_dt= px.bar(dt2, x= "Transaction_name", y= "Transaction_amount", title= "DISTRICTS WITH LOWEST TRANSACTION AMOUNT",
...
with tab2:
    ...
    if sel_year == "All" :
        ...
        state=st.selectbox("selecet the state",('Andaman & Nicobar', 'Andhra Pradesh', 'Arunachal Pradesh',
                                                ...
                                                'Dadra and Nagar and daman and diu', 'Delhi', 'Goa',
